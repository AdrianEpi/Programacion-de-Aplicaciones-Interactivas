...
/**
 * Calcula los tiros libres que realiza cada equipo
 * @param {object} PARTIDO 
 */
function tirosLibres(PARTIDO) {
  let tirosHawks = 0;
  let tirosPacers = 0;
  for (firstName in PARTIDO.players) {
    if (parseInt(PARTIDO.players[firstName].freeThrowsAttempted) > 0) {
      if (PARTIDO.players[firstName].teamName === 'Hawks') {
        tirosHawks++;
      }
      else {
        tirosPacers++;
  ...
  console.log('Hawks attempted the most free throws... Pacers:', tirosPacers, 'Hawks:', tirosHawks);
}